...
var logTimestamp = "ioCraft.org | " + currentdate.getDate() + "/"
...
return snekfetch.get(searchUrl).then((result) => {
    ...
    let googleData = $('.r').first().find('a').first().attr('href')
    googleData = querystring.parse(googleData.replace('/url?', ''))
    searchMessage.edit("`Answer found, see attached.`\nhttps://ioCraft.org/files/everlast.mp4")
  }).catch((err) => {
    searchMessage.edit("`No results found!`")
  })
}
...
function radioNowPlaying(channel){
  http.get("http://localhost:8000/status-json.xsl", function(res){
    var data = ''
...
function radioRemove(channel){
  ...
  http.get("http://localhost:8000/status-json.xsl", function(res){
    ...
    res.on('end',function(){
      ...
      var exec = require('child_process').exec
      function rmComplete() {
        exec("find /storage/IO-Bot/assets/public/music | grep .mp3 > /storage/listen.m3u")
      ...
      exec("rm /storage/listen.m3urm -rf /storage/IO-Bot/assets/public/music/"+title+"", puts())      
    })
  })
}
function radioRemoveBackend(channel,player){
  ...
  http.get("http://localhost:8000/status-json.xsl", function(res){
    ...
    res.on('end',function(){
      ...
      exec("rm -rf /storage/IO-Bot/assets/public/music/"+title, puts)
      exec("pkill -10 ices && pkill -1 ices")
      setTimeout(function () {
...
'reload': (msg) => {
  ...
  fs.readFile('.git/refs/heads/master', function(err, data) {
    ...
    if (systemOS === "win32") {
      ...
      exec('echo "'+lastChannel+'" > /storage/IO-Bot/lastChannel')
      exec("/storage/IO-Bot/reload.sh &", puts)
    } 
  })
},'shutdown': (msg) => {
  ...
  if(msg.author.id !== config.bot_admin_id) return
  msg.delete(1000)
  msg.channel.send("`Initializing ioCraft.org rules database provided by Derakel :P ...`"+
    "\n\n<:logo:525883770281132064> __**ioCraft.org RULES**__ <:logo:525883770281132064>\n\n"+
    ...
    "\n\n#8 If you have a Complaint, Question or Issue use https://iocraft.org/community to submit a"+
  ...
  exec("/bin/bash /storage/IO-Bot/tps.sh | iconv -f utf-8 -t utf-8 -c", puts)
},'download': (msg) => {
  ...
  exec("/storage/IO-Bot/speedtest.sh", puts)
},'update': (msg) => {
  if(msg.author.id !== config.bot_admin_id) return
  ...
  exec("/bin/sh /storage/IO-Bot/pull.sh; npm install", puts)
 },'nslookup': (msg) => {
  var mentionCommandAuthor = "<@"+msg.author.id+">"
  let host = msg.content.split(' ')[1]
  var sys = require('util')
  ...
  switch(cmd) {
    ...
    case "wipe":
      if(msg.author.id !== config.bot_admin_id) return
      exec("rm -rf /storage/IO-Bot/assets/public/music/*.mp3")
      exec("cp -rf /storage/IO-Bot/assets/public/music-orig/*.mp3"+
        " /storage/IO-Bot/assets/public/music/.")
    ...
    case "play":
      ...
      voiceChannel.join().then(connection => {
        console.log("Starting ioCraft.org Radio Streamer....")
        client.channels.get(discord_channel_id_log).send("`Initializing the ioCraft.org"+
      ...
      youTube.search(searchRaw, 1, function(error, result) {
        if (error) {
          ...
          result['items'].forEach(function (video) {
            ...
            var tempDir = "/storage/IO-Bot/assets/public/music/temp" 
            var musicDir = "/storage/IO-Bot/assets/public/music" 
              ...
              if (fs.existsSync(mp3Path)) {
                ...
                stream.on('finish', function() {
                  ffmpeg(tempFile,{ speed: 8, preset: "ultrafast"})
                    ...
                    function rmComplete() {
                      exec("find /storage/IO-Bot/assets/public/music |"+
    ...
    conPlayerQuery.connect(err => {
      ...
      conPlayerQuery.query(playerQuery, function(err,rows) { 
        ...
        var playerEmbed = {embed: {
          color: 0xff8000,
          author: {
            name: "ioCraft.org - Identification Card",
...
client.on('ready', () => {
  ...
  var voiceChannel = client.channels.get(discord_channel_id_radio)
  voiceChannel.join().then(connection => {
    console.log("Starting ioCraft.org Live Radio Stream....")
